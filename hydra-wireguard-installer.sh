...
cat << "IGNARE"
  _   _           _             _    _           _   _             
 | | | |_   _  __| |_ __ __ _  | |  | | ___  ___| |_(_)_ __   __ _ 
 | |_| | | | |/ _` | '__/ _` | | |__| |/ _ \/ __| __| | '_ \ / _` |
 |  _  | |_| | (_| | | | (_| | |  __  | (_) \__ \ |_| | | | | (_| |
 |_| |_|\__, |\__,_|_|  \__,_| |_|  |_|\___/|___/\__|_|_| |_|\__, |
        |___/                                                |___/ 
IGNARE
echo -e "${NC}"
echo -e "${WHITE}================================================================${NC}"
echo -e "${RED}           WireGuard VPN Installer by Hydra Hosting             ${NC}"
echo -e "${WHITE}================================================================${NC}\n"
}
...
new_client_dns () {
	echo -e "${RED}Select a DNS server for the client:${NC}"
	echo -e "${WHITE}   1) Default system resolvers${NC}"
	echo -e "${WHITE}   2) Google${NC}"
	echo -e "${WHITE}   3) 1.1.1.1${NC}"
	echo -e "${WHITE}   4) OpenDNS${NC}"
	echo -e "${WHITE}   5) Quad9${NC}"
	echo -e "${WHITE}   6) Gcore${NC}"
	echo -e "${WHITE}   7) AdGuard${NC}"
	echo -e "${WHITE}   8) Specify custom resolvers${NC}"
	read -p "$(echo -e \"${CYAN}DNS server [1]: ${NC}\") " dns
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
	...
	if ! hash wget 2>/dev/null && ! hash curl 2>/dev/null; then
		...
		read -n1 -r -p "$(echo -e \"${YELLOW}Press any key to install Wget and continue...${NC}\") "
		apt-get update
		apt-get install -y wget
	fi
	clear
	print_logo
	echo -e "${CYAN}Welcome to the Hydra Hosting WireGuard installer!${NC}"
	...
	if [[ $(ip -4 addr | grep inet | grep -vEc '127(\.[0-9]{1,3}){3}') -eq 1 ]]; then
		...
		echo
		echo -e "${RED}Which IPv4 address should be used?${NC}"
		ip -4 addr | grep inet | grep -vE '127(\.[0-9]{1,3}){3}' | cut -d '/' -f 1 | grep -oE '[0-9]{1,3}(\.[0-9]{1,3}){3}' | nl -s ') '
		read -p "$(echo -e \"${CYAN}IPv4 address [1]: ${NC}\") " ip_number
	...
	if echo "$ip" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		...
		read -p "$(echo -e \"${CYAN}Public IPv4 address / hostname [$get_public_ip]: ${NC}\") " public_ip
	...
	if [[ $(ip -6 addr | grep -c 'inet6 [23]') -gt 1 ]]; then
		...
		echo
		echo -e "${RED}Which IPv6 address should be used?${NC}"
		ip -6 addr | grep 'inet6 [23]' | cut -d '/' -f 1 | grep -oE '([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}' | nl -s ') '
		read -p "$(echo -e \"${CYAN}IPv6 address [1]: ${NC}\") " ip6_number
	...
	echo
	echo -e "${RED}What port should WireGuard listen on?${NC}"
	read -p "$(echo -e \"${CYAN}Port [51820]: ${NC}\") " port
	...
	[[ -z "$port" ]] && port="51820"
	echo
	echo -e "${RED}Enter a name for the first client:${NC}"
	read -p "$(echo -e \"${CYAN}Name [client]: ${NC}\") " unsanitized_client
	...
	if [[ "$use_boringtun" -eq 1 ]]; then
		...
		read -p "$(echo -e \"${CYAN}Should automatic updates be enabled for it? [Y/n]: ${NC}\") " boringtun_updates
	...
	echo
	echo -e "${GREEN}WireGuard installation is ready to begin.${NC}"
	...
	read -n1 -r -p "$(echo -e \"${YELLOW}Press any key to continue...${NC}\") "
	# Install WireGuard
...
EOF
	...
	echo
	echo -e "${WHITE}The client configuration is available in: ${CYAN}$script_dir/$client.conf${NC}"
	echo "New clients can be added by running this script again."
else
	clear
	print_logo
	echo -e "${CYAN}WireGuard is already installed.${NC}"
	echo
	echo -e "${RED}Select an option:${NC}"
	echo -e "${WHITE}   1) Add a new client${NC}"
	echo -e "${WHITE}   2) Remove an existing client${NC}"
	echo -e "${WHITE}   3) Remove WireGuard${NC}"
	echo -e "${WHITE}   4) Exit${NC}"
	read -p "$(echo -e \"${CYAN}Option: ${NC}\") " option
	until [[ "$option" =~ ^[1-4]$ ]]; do
	...
	case "$option" in
		1)
			...
			read -p "$(echo -e \"${CYAN}Name: ${NC}\") " unsanitized_client
			...
			echo
			echo -e "${WHITE}$client added. Configuration available in: ${CYAN}$script_dir/$client.conf${NC}"
			exit
		;;
		2)
			...
			read -p "$(echo -e \"${CYAN}Client: ${NC}\") " client_number
			...
			echo
			read -p "$(echo -e \"${CYAN}Confirm $client removal? [y/N]: ${NC}\") " remove
		...
		3)
			echo
			read -p "$(echo -e \"${CYAN}Confirm WireGuard removal? [y/N]: ${NC}\") " remove
			until [[ "$remove" =~ ^[yYnN]*$ ]]; do
				echo "$remove: invalid selection."
				read -p "$(echo -e \"${CYAN}Confirm WireGuard removal? [y/N]: ${NC}\") " remove
			done
